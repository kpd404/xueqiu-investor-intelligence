...
class OpenAICompatibleOpinionExtractor(OpinionExtractor):
    ...
    def _structured_output_config(cls) -> dict[str, object]:
        ...
        if isinstance(properties, dict):
            # Provider metadata is attached from the SDK response after
            # parsing and is not part of the model-authored extraction.
            properties.pop("provider_metadata", None)
        if isinstance(required, list):
            schema["required"] = [name for name in required if name != "provider_metadata"]
        return {
            "format": {
    ...
    def _make_strict_schema(
        ...
        if isinstance(node, dict):
            ...
            for value in node.values():
                OpenAICompatibleOpinionExtractor._make_strict_schema(value, local_definitions)
